"""
문제 : N개의 도시가 있다. 그리고 한 도시에서 출발하여 다른 도시에 도착하는 M개의 버스가 있다. 
...
def djk_search(str) :
    ...
    while Q :
        time, node= heapq.heappop(Q) #가중치가 가장 적은것을 pop

        if node not in dist :
            dist[node] = time
            for end, cost in arr[node] :
                alt = time + cost
                heapq.heappush(Q, (alt, end))


N = int(input("도시의 개수를 입력하세요 : "))
M = int(input("버스의 개수를 입력하세요 : "))

arr = collections.defaultdict(list)

for _ in range(M) : 
    start, end, cost = map(int, input("출발 도시의 번호, 도착지의 도시 번호, 버스의 비용을 입력하세요 : ").split())
    arr[start].append((end, cost))

strPoint, desPoint = map(int, input("구하고자 하는 구간의 출발점의 도시번호와 도착점의 도시번호를 입력하세요 : ").split())
...
print(strPoint,"번째 도시부터 ",desPoint,"번째 도시까지의 최소 비용 : ",dist[desPoint])